...
class OpenAIModelInterface(BaseModelInterface):
    ...
    def __init__(self, config: OpenAIModelConfig):
        ...
        self.config = config
        # self.llm = LLM(model=config.model_name,tensor_parallel_size=8,trust_remote_code=True,max_num_batched_tokens=128000)
    ...
    def generate(self, prompts: List[Any], **kwargs) -> List[Dict[str, Any]]:
        ...
        # sampling_params = [SamplingParams(temperature=kwargs.get("temperature", self.config.temperature)) for _ in range(len(formatted_requests))]
        # outputs = self.llm.chat(messages=formatted_requests,sampling_params=sampling_params,use_tqdm=True)
        # results = []
        ...
        return results
    # async def generate(self, prompts: List[Any], **kwargs) -> List[Dict[str, Any]]:
    #     """Asynchronous version of generate using OpenAI API."""
    #     formatted_requests = []
    #     for prompt in prompts:
    #         messages = self._convert_qwen_to_openai_format(prompt)
    #         formatted_requests.append(messages)

    #     async def call_single(messages: List[Dict]):
    #         try:
    ...
    #     # 并发执行所有请求
    #     tasks = [call_single(messages) for messages in formatted_requests]
    ...
    def _convert_qwen_to_openai_format(self, prompt: List[Dict]) -> List[Dict]:
        ...
        # 2. 拿到非 system 的对话
        dialogue = [msg for msg in prompt if msg.get("role") != "system"]
        dialogue = dialogue[-(2*self.config.window_size - 1):]  # Keep only last N exchanges
        
        new_prompt = [system_prompt]+dialogue if system_prompt else dialogue
