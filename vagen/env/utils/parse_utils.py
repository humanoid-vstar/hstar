...
def parse_freethink(response: str, special_token_list=None, action_sep=',', max_actions=3) -> Dict:
    """
    Parse response. Robustly extracts answer even if <think> is missing or format is imperfect.
    """
    response = response.replace("<image>", "")
    ...
    extraction_pattern = r'(?:<think>(.*?)</think>)?.*?<answer>(.*?)</answer>'
    match = re.search(extraction_pattern, response, re.DOTALL)
    
    think_content, action_content, actions = "", "", []

    if match:
        think_raw = match.group(1)
        action_raw = match.group(2)
        
        think_content = think_raw if think_raw else ""
        action_content = action_raw if action_raw else ""
