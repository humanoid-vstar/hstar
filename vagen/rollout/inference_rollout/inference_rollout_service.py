...
class InferenceRolloutService(BaseRollout):
    ...
    def reset(self, env_configs: List[Dict]) -> Dict[str, Tuple[Dict, Dict]]:
        ...
        for i, cfg in enumerate(env_configs):
            env_id = f"{self.split}_{cfg.get('seed', i)}"
    ...
    def _generate_batch_responses(self, env_messages: Dict[str, List[Dict]]) -> Dict[str, str]:
        ...
        gen_time = time.time() - start_time
        print(f"[DEBUG] Generated {len(prompts)} responses in {gen_time:.3f}s")
    ...
    def recording_to_log(self) -> List[Dict]:
        ...
        for env_id in self.envs:
                ...
                if "multi_modal_data" in message:
                    for key, values in message["multi_modal_data"].items():
                        if key == "<image>" or "image" in key.lower():
                            for value in values:
                                # Handle different image formats to base64
                                if isinstance(value, PIL.Image.Image):
                                    # from vagen.server.serial import serialize_pil_image
                                    # serialized_image = serialize_pil_image(value)
                                    path = f"{self.output_dir}/result_images/{env_id}_{count}.png"
                                    value.save(path, format="PNG")
                                    count+=1
                                    # if isinstance(serialized_image, dict) and "__pil_image__" in serialized_image:
                                ...
                                # Handle serialized images from the service
                                    ...
                                    pil_images = deserialize_pil_image(value['__pil_image__'])
                                    path = f"{self.output_dir}/result_images/{env_id}_{count}.png"
                                    pil_images.save(path, format="PNG")
                                    count+=1
                                    image_data.append(path)
            ...
            print(f"[SCORE DEBUG] env_id={env_id}, steps={convert_numpy_types(step_count)}, success={self.env_states[env_id]['metrics']['traj_metrics'].get('success', False)}), done={done}")
